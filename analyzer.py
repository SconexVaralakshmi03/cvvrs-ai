...
# # #                     timestamp_seconds        = _fmt_duration(journey_ts),  # FIX: format as H:MM:SS string for API
...
# # ─────────────────────────────────────────
# # Previously the video-iteration loop had no try/except around pipeline.run().
# # Any exception raised for one video — including SystemExit raised by
# # GadgetDetectionPipeline.run() when cv2.VideoCapture cannot open a file —
# # propagated uncaught through analyze_journey() into consumer._handle_job()'s
# # outer except block, which called send_failed() and nacked the entire message,
# # failing all videos even those that had not been attempted yet.

# # Fix: each pipeline.run() call is now wrapped in its own try/except BaseException
# # block.  On failure the video is skipped (with a WARNING log), time/frame offsets
# # are advanced using the metadata we already captured, and processing continues
# # with the next video.  The VideoResult for the failed video is still included in
# # the output (with zero violations) so the completion callback has a full picture.

# # Only SystemExit (sys.exit) and KeyboardInterrupt propagate out of the loop:
# #   • KeyboardInterrupt — operator pressed Ctrl-C; let it surface.
# #   • SystemExit        — treated as a fatal signal; re-raise so the consumer
# #                         nacks and the job goes to the DLQ rather than being
# #                         silently swallowed.
# # All other exceptions (including any BaseException subclasses) are caught and
# # logged at the per-video level.
# # """
