...
def analyze_journey(
    ...
    try:
        ...
        for v in [v for v in shared_vstore._violations if "hand_raise" in v.events]:
            ...
            frames, frame_timestamps = extract_frame_window(tmp_path, local_secs, fps)
            ...
            verdict = verify_rsl_hand_brake(
                frames, timestamps=frame_timestamps,
                log_label=f"rsl_hand_brake:{src_file}@{local_secs:.2f}s",
            )
    ...
    for vj in ordered:
        ...
        # NEW — send violations/frames to the Java backend in ASCENDING
        # timestamp order. shared_vstore._violations can end up with
        # violations out of chronological order (e.g. multiple detectors
        # running concurrently, merge/dedup re-inserting an episode's
        # final record after later ones were already appended) — e.g.
        # 00:01:28, 00:01:29, 00:00:21 as recorded, which must go out as
        # 00:00:21, 00:01:28, 00:01:29. Sort by the numeric seconds value
        # (not the "H:MM:SS" string, which doesn't sort correctly once
        # hour digits differ) before building violation_results below.
        raw_viols = sorted(
            raw_viols,
            key=lambda v: _hms_to_seconds(getattr(v, "local_time_str", "0:00:00")),
        )
