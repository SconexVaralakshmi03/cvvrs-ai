...
#     """
#     Return ALL rows where process_flag = 'N' across every train.
#     No train_detail_id filter — picks up everything pending.

#     Ordered by train_detail_id, folder_name, seq_no so:
#       - each train's folders are processed together
#       - within each folder videos are in the correct sequence
#     """
...
#                        filename, s3_video_path, seq_no
#                 FROM   video_files
#                 WHERE  process_flag = 'N'
#                 ORDER  BY train_detail_id, folder_name, seq_no
#                 """
#             )
#             rows = cur.fetchall()
#             print(f"[DB] Found {len(rows)} pending video(s) with process_flag='N'")
#             for r in rows:
#                 print(f"     train={r['train_detail_id']}  "
...
#     """
#     N = pending   I = in progress   Y = done
#     On pipeline failure this is NOT called — flag stays 'I'
#     so the operator can see exactly which video failed.
#     """
...
#     """Store S3 result path on every row in this folder after processing."""
#     conn = get_db_connection()
#     try:
#         with conn.cursor() as cur:
#             cur.execute(
...
#     """Download video from S3 to local_path."""
...
#     """
#     Upload the full output folder to S3 after pipeline finishes.

#     Local:   outputs/<folder_name>/analysis_report.json
#              outputs/<folder_name>/frames/*.jpg
#     S3:      <folder_name>/results/analysis_report.json
#              <folder_name>/results/frames/*.jpg

#     Returns the S3 URI of analysis_report.json.
#     """
#     s3        = _s3_client()
#     bkt       = _bucket()
...
# # ══════════════════════════════════════════════════════════════════
# # CALLED BY violation_store.ViolationStore.finalize()
# # ══════════════════════════════════════════════════════════════════
...
# ) -> None:
#     """
#     Called automatically by ViolationStore.finalize() after JSON
#     report and frame images are written locally.
#     Uploads results to S3 and stores the path back in DB.
#     """
...
def set_process_flag(video_id: int, flag: str) -> None:
    """
    Update process_flag for a single video row.
      N = pending      (set by frontend/ingestion)
      Q = queued       (set on sibling videos when the first video in a folder starts)
      I = in-progress  (set on the single video currently being processed)
      Y = done         (set after pipeline completes successfully)

    On pipeline failure this is intentionally NOT called with 'Y' —
    the flag stays 'I' so operators can see which video failed.
    """
    assert flag in ("N", "I", "Q", "Y"), f"Invalid flag: {flag!r}"
    conn = get_db_connection()
    try:
        with conn.cursor() as cur:
            cur.execute(
