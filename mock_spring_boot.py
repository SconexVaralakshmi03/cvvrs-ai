...
• /completed now prints and validates the new fields:

    - videoName, durationFormatted, fps, sizeMb  (per video)

    - timestamp, originalVideoTimestamp           (per violation)

• Validation warnings printed if any expected fields are missing.

• Progress endpoint prints currentVideo index if present.
