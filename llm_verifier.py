...
Consensus (repeat-inference) verification layer — NEW
---------------------------------------------------------------------------
The deterministic override above is only as reliable as the model's own
self-reported structured fields (hand_is_elevated_in_air, arm_direction,
device_hardware_visible, etc.) — it cannot independently re-examine the
image. For a small quantized VLM (qwen2.5vl:7b), those fields are not
always consistent call-to-call on a genuinely borderline frame: the same
image can get "hand_height: above_head" on one call and
"hand_height: at_desk_level" on the next.

To catch this, Mobile Phone / Hand Raising / Seat Absence (the three
types with an override layer) are now, by default, sent to the model
LLM_CONSENSUS_RUNS times (config/settings.py, default 2) instead of once.
The candidate is only accepted if EVERY run independently parses
successfully AND agrees verified=True after its own override check —
any disagreement, error, or unparseable run is treated as doubt and the
whole candidate is rejected. This only ever makes rejection MORE likely
than a single pass would (same "when in doubt, reject" philosophy as the
override layer above), and it never changes which frames get sent for
verification. Drowsiness and RSL Hand Brake are unaffected (no override
layer is defined for them, so this failure mode doesn't apply to them).
Toggle: LLM_CONSENSUS_VERIFICATION_ENABLED (default True).
"""
...
# Consensus (repeat-inference) verification layer (see module docstring
# above). Default on; can be disabled to fall back to a single model call
# per candidate, exactly as before this change.
LLM_CONSENSUS_VERIFICATION_ENABLED = bool(_cfg("LLM_CONSENSUS_VERIFICATION_ENABLED", True))
LLM_CONSENSUS_RUNS = max(1, int(_cfg("LLM_CONSENSUS_RUNS", 2)))

# Only the violation types with a deterministic override layer are in
# scope — their accept/reject outcome is driven by the model's own
# self-reported structured fields, which is exactly what can disagree
# between repeat calls on a borderline frame. Drowsiness and RSL Hand
# Brake have no override layer, so this specific failure mode does not
# apply to them; they are intentionally left on a single call.
LLM_CONSENSUS_VIOLATION_TYPES = frozenset({
...
def _fetch_and_verify_once(
    ...
    """
    One full call-model → parse → deterministic-override pass.

    This is the exact same logic verify_frame() always ran inline, just
    extracted so it can be invoked either once (the default, unchanged
    behaviour for Drowsiness/RSL and whenever consensus verification is
    disabled) or multiple times in a row for the consensus check below.
    It does NOT apply the fail-open/fail-closed policy or emit the audit
    log line — the caller does that once, after collecting every run.

    Returns a dict:
        {
          "ok":         bool,  # True iff a model response was received
                                # AND parsed as valid JSON. False means
                                # this run is unusable (network/timeout/
                                # unparseable) and should count as a "no"
                                # for consensus purposes.
          "verified":   bool,
          "role":       str,   # raw role string, e.g. "Loco Pilot" —
    ...
    """
    label = f"{log_label}{run_suffix}"
    raw_text: Optional[str] = None
    last_error: Optional[Exception] = None
    for attempt in range(1, OLLAMA_MAX_RETRIES + 2):
        try:
            raw_text = _call_ollama(prompt_text, image_bgr, log_label=label)
            break
        except Exception as exc:
            last_error = exc
            log.warning(
                "LLM verify attempt %d/%d failed for %s (%s): %s",
                attempt, OLLAMA_MAX_RETRIES + 1, label, candidate_violation, exc,
            )

    if raw_text is None:
        log.error(
            "LLM verification unavailable for %s (%s) after retries: %s",
            label, candidate_violation, last_error,
        )
        return {
            "ok": False, "verified": False, "role": "Unknown", "confidence": 0,
            "reason": f"LLM verification unavailable: {last_error}", "raw_text": None,
        }

    parsed = _safe_parse(raw_text)
    if parsed is None:
        log.warning("LLM returned unparseable JSON for %s (%s)", label, candidate_violation)
        return {
            "ok": False, "verified": False, "role": "Unknown", "confidence": 0,
            "reason": "Model response could not be parsed as valid JSON.",
            "raw_text": raw_text,
    ...
    if (override_verified, override_role, override_confidence, override_reason) != (
        ...
        log.info(
            "Deterministic override changed verdict for %s (%s): "
            "verified %s -> %s (%s)",
            label, candidate_violation, parsed["verified"], override_verified,
            override_reason,
        )

    print(f"[llm_verifier] PARSED verdict for {label!r} (candidate={candidate_violation}): "
          f"verified={override_verified} role={override_role} "
          f"confidence={override_confidence} reason={override_reason!r} raw_fields={parsed}")

    return {
        "ok": True,
        "verified": bool(override_verified),
        "role": override_role,
...
def verify_frame(image_bgr: np.ndarray, event_type: str, log_label: str = "") -> dict:
    ...
    For Mobile Phone / Hand Raising / Seat Absence (the violation types
    with a deterministic override layer), the model is called
    LLM_CONSENSUS_RUNS times instead of once, and the candidate is only
    accepted if EVERY run agrees verified=True — see module docstring
    ("Consensus (repeat-inference) verification layer") for why. All
    other violation types (Drowsiness) are unaffected and still use a
    ...
    # ── How many times do we call the model for this candidate? ──────────
    # Consensus applies only to the violation types with an override layer
    # (see module docstring) and only when the toggle is on; every other
    # case is a single call, unchanged from before this feature existed.
    run_count = (
        LLM_CONSENSUS_RUNS
        if (LLM_CONSENSUS_VERIFICATION_ENABLED and candidate_violation in LLM_CONSENSUS_VIOLATION_TYPES)
        else 1
    )

    passes = [
        _fetch_and_verify_once(
            prompt_text, image_bgr, candidate_violation, log_label,
            run_suffix="" if run_count == 1 else f"#run{i + 1}/{run_count}",
        )
        for i in range(run_count)
    ]
    ok_passes = [p for p in passes if p["ok"]]

    if not ok_passes:
        # Every run failed to reach/parse the LLM — fall back to the
        # existing fail-open/fail-closed policy, unchanged from before
        # this feature existed.
        fail_open = LLM_VERIFICATION_FAIL_OPEN
        status, role = _status_role(fail_open, "Unknown")
        reason = passes[-1]["reason"]
        log.error(
            "LLM verification unavailable for %s (%s) after %d run(s): %s",
            log_label, candidate_violation, run_count,
            "failing OPEN (kept, unverified)" if fail_open else "failing CLOSED (rejected)",
        )
        _log_verification_json(
            label=log_label, event_type=event_type, candidate_violation=candidate_violation,
            llm_invoked=True, status=status, role=role, confidence=0, reason=reason,
            skipped=False, consensus_runs=run_count, consensus_ok_runs=0,
        )
        return {
            "verified": fail_open, "status": status, "role": role, "confidence": 0,
            "reason": reason, "skipped": False, "llm_invoked": True,
    ...
    if run_count == 1:
        # Unchanged single-pass behaviour.
        result = ok_passes[0]
        verified, role, confidence, reason = (
            result["verified"], result["role"], result["confidence"], result["reason"],
        )
    else:
        # ── Consensus rule ────────────────────────────────────────────
        # Accept only if EVERY run reached the model, parsed successfully,
        # AND agreed verified=True after its own override check. Any run
        # that disagrees, errors out, or is unparseable counts as a "no"
        # — this only ever makes rejection MORE likely than a single pass
        # would, matching the existing "when in doubt, reject" design.
        all_agree_true = (len(ok_passes) == run_count) and all(p["verified"] for p in ok_passes)
        if not all_agree_true:
            confirmed = sum(1 for p in ok_passes if p["verified"])
            verified, role, confidence = False, "Unknown", 0
            reason = (
                f"Overridden to REJECTED: consensus check failed "
                f"({confirmed}/{run_count} run(s) confirmed the candidate; "
                f"{run_count - len(ok_passes)} run(s) errored or were unparseable)."
            )
            log.info(
                "Consensus check REJECTED %s (%s): %d/%d runs confirmed.",
                log_label, candidate_violation, confirmed, run_count,
            )
        else:
            verified = True
            roles = {p["role"] for p in ok_passes if p["role"] not in (None, "Unknown")}
            role = roles.pop() if len(roles) == 1 else "Unknown"
            confidence = min(p["confidence"] for p in ok_passes)
            reason = (
                f"ACCEPTED: {run_count}/{run_count} independent verification "
                f"runs agreed the candidate is confirmed."
            )

    status, out_role = _status_role(verified, role)
    result_dict = {
        "verified": verified, "status": status, "role": out_role, "confidence": confidence,
        "reason": reason, "skipped": False, "llm_invoked": True,
    }
    print(f"[llm_verifier] FINAL verdict for {log_label!r} "
          f"(candidate={candidate_violation}, consensus_runs={run_count}, "
          f"consensus_ok_runs={len(ok_passes)}): {result_dict}")
    _log_verification_json(
        label=log_label, event_type=event_type, candidate_violation=candidate_violation,
        llm_invoked=True, status=status, role=out_role, confidence=confidence,
        reason=reason, skipped=False, consensus_runs=run_count,
        consensus_ok_runs=len(ok_passes),
    )
    return result_dict
