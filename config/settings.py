...
OLLAMA_TIMEOUT_SECONDS        = 45
OLLAMA_MAX_RETRIES            = 1
# Was 0.1. Lowered to 0 (greedy decoding) to remove one source of
# run-to-run non-determinism in the model's structured-field judgments
# (e.g. hand_height / arm_direction flipping between identical calls on
# the same frame). Note: even at temperature 0, a quantized 7B VLM is not
# perfectly deterministic call-to-call -- this alone does not fully solve
# that instability, which is why LLM_CONSENSUS_VERIFICATION_ENABLED below
# exists as the primary fix.
OLLAMA_TEMPERATURE            = 0

# ── Consensus (repeat-inference) verification — NEW ────────────────────────
# For the violation types that have a deterministic override layer in
# llm_verifier.py (Mobile Phone, Hand Raising, Seat Absence), the model is
# called LLM_CONSENSUS_RUNS times instead of once, and the candidate is
# only accepted if EVERY run independently agrees verified=True after its
# own override check. This exists because those three types' accept/reject
# outcome is driven by the model's own self-reported structured fields
# (hand_is_elevated_in_air, arm_direction, device_hardware_visible, etc.),
# and a small quantized VLM can give a different answer to the identical
# question on a second call for a borderline frame. Requiring agreement
# across repeat calls turns "the model happened to hallucinate once" into
# "the model hallucinated consistently" before it's trusted -- any
# disagreement is treated as doubt and the candidate is rejected, matching
# the existing "when in doubt, reject" design. See llm_verifier.py's
# _fetch_and_verify_once() / verify_frame().
LLM_CONSENSUS_VERIFICATION_ENABLED = True
