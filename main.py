...
# # # # #                and self._processed_frame_no % 500 == 0):
# # # # #                     try:
# # # # #                         if self._processed_frame_no % 500 == 0:
# # # # #                             print(
# # # # #                            f"[RabbitMQ heartbeat] frame={self._processed_frame_no}")
# # # # #                             try:
# # # # #                                 self.rabbit_connection.process_data_events(time_limit=0)
...
# # # #         self._prev_frame_detections = None
# # # #         self._processed_frame_no    = 0
# # # #         self._absence_pilot_boxes: list = [          # ← change this line
# # # #     (2, (0, 0,   848, 274)),                 #   Pilot 2 = upper zone (274 = 480×0.57)
# # # #     (1, (0, 274, 848, 480)),                 #   Pilot 1 = lower zone
# # # # ]
# # # #         self._yolo_empty_run_count: int = 0
...
# # # #         if not cap.isOpened():
# # # #             self.logger.error(f"Cannot open source: {self.source!r}")
# # # #             # FIX — Issue #2: raise RuntimeError instead of calling sys.exit(1).
# # # #             # sys.exit() raises SystemExit (a BaseException subclass) which
# # # #             # escapes "except Exception" guards in analyzer.py and consumer.py,
# # # #             # causing the entire journey to be failed when only one video file
# # # #             # cannot be opened.  RuntimeError is caught by the per-video
# # # #             # try/except BaseException block in analyzer.py, allowing the
# # # #             # remaining videos to continue processing normally.
...
# # #         self._prev_frame_detections = None
# # #         self._processed_frame_no    = 0
# # #         # Pre-fill with full-zone boxes so absence detector considers both pilots
# # #         # seated from frame 0, before YOLO has had its first successful run.
# # #         # Prevents spurious alerts at the start of every video in a journey.
# # #         split_y = int(480 * 0.57)  # 273 — updated to actual dims in run()
# # #         self._absence_pilot_boxes: list = [
# # #             (2, (0, 0, 848, split_y)),
# # #             (1, (0, split_y,  848, 480)),
# # #         ]
# # #         self._yolo_empty_run_count: int = 0
# # #         self._pilot_is_seated: dict     = {1: True, 2: True}
...
# # #                 # Track seated state per pilot for drowsiness suppression
# # #                 if droop_results:
# # #                     for dr in droop_results:
# # #                         self._pilot_is_seated[dr.pilot_id] = dr.is_seated
# # #         except Exception as exc:
# # #             self.logger.error(f"Droop error frame {global_frame}: {exc}", exc_info=True)
...
# # #         if droop_log_events:
# # #             # Only suppress drowsiness for a pilot who is THEMSELVES using a phone.
# # #             # Looking down at a phone causes false forward_droop for that same pilot.
# # #             # Do NOT suppress based on absence — if Pilot 1 is absent, Pilot 2 can
# # #             # still be sleeping and must be detected independently.
# # #             phone_pids = {e[0] for e in log_events} if log_events else set()

# # #             filtered_droop_events = [
# # #                 e for e in droop_log_events
# # #                 if e[0] not in phone_pids
# # #             ]

# # #             if filtered_droop_events:
# # #                 # Events are Tuple[int, str] where str format is "DROWSINESS:TYPE"
# # #                 messages    = [e[1] for e in filtered_droop_events]
# # #                 is_sleeping = any("EYES" in m for m in messages)
# # #                 dtype       = messages[0].split(":")[-1] if ":" in messages[0] else "UNKNOWN"
...
# # #                 dr_ref = next(
# # #                     (dr for dr in droop_results
# # #                      if dr.drooping and dr.pilot_id not in phone_pids),
# # #                     None,
# # #                 )
# # #                 dur_drp      = dr_ref.timer_value if dr_ref else 0.0
# # #                 event_global = max(0.0, global_time - HEAD_DROP_DURATION)
...
# #         if droop_log_events:
# #             # ── MERGED LOGIC (keeps both behaviors, drops neither) ───────────
# #             #
# #             # 1. PHONE-EXCLUSION (older logic): a pilot who is themselves
# #             #    using a phone right now gets their drowsiness event
# #             #    suppressed for this frame — looking down at a phone causes
# #             #    a false forward_droop reading for that same pilot. This
# #             #    does NOT apply across pilots — if Pilot 1 is on a phone,
# #             #    Pilot 2 can still be flagged sleeping independently.
# #             #
# #             # 2. ABSENCE-OVERLAP (newer logic): if the SAME pilot is flagged
# #             #    both drowsy and absent in this frame (e.g. slumped low
# #             #    enough that seat-absence also triggers), classify the
# #             #    event as "sleeping_absent" rather than plain "drowsy"/
# #             #    "sleeping".
# #             #
# #             # 3. DROOP_TYPE messaging (older logic): the specific posture
# #             #    that triggered the alert (FORWARD / BACKWARD / EYES) is
# #             #    surfaced in the human-readable event message.
# #             phone_pids = {e[0] for e in log_events} if log_events else set()
...
# #             if filtered_droop_events:
# #                 # Events are Tuple[int, str] where str format is "DROWSINESS:TYPE"
# #                 droop_pids  = {e[0] for e in filtered_droop_events}
# #                 messages    = [e[1] for e in filtered_droop_events]
# #                 is_sleeping = any("EYES" in m for m in messages)
# #                 dtype       = messages[0].split(":")[-1] if ":" in messages[0] else "UNKNOWN"
...
#     # ──────────────────────────────────────────────────────────────────────────
#     # _grab_frame_at  — recover the exact frame at a given LOCAL video time
#     # ──────────────────────────────────────────────────────────────────────────
#     #
#     # Used for violations whose video_time/local_video_time is back-dated
#     # (e.g. seat_absence, which logs ABSENCE_ALLOWED_DURATION seconds after
#     # the pilot actually left — see the absence_log_events block below).
#     # The frame the pilot actually left in is no longer in memory by the
#     # time the violation fires, because frames flow through the reader
#     # thread's queue and are discarded once consumed. Re-opening the source
#     # file and seeking is the only reliable way to recover that exact frame.
#     #
#     # Opens its own short-lived cv2.VideoCapture rather than touching the
#     # pipeline's main `cap` — this runs from the same thread that owns
#     # `cap` (no concurrent-seek hazard), but using a separate handle keeps
#     # the main capture's read position untouched for the reader thread.
#     def _grab_frame_at(self, source, local_seconds: float):
#         """
#         Seek `source` (a video file path) to `local_seconds` and return that
#         frame, or None if the seek/read fails.
#         """
#         cap = cv2.VideoCapture(source)
#         if not cap.isOpened():
#             return None
#         try:
#             cap.set(cv2.CAP_PROP_POS_MSEC, max(0.0, local_seconds) * 1000.0)
#             ret, frame = cap.read()
#             if not ret:
#                 # Fallback: seek by frame number instead of msec
#                 fps   = cap.get(cv2.CAP_PROP_FPS) or 25.0
#                 total = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
#                 target_frame = min(int(max(0.0, local_seconds) * fps), max(0, total - 1))
...
#             # FIX — save the frame at the moment the phone was actually
#             # first detected, not the live frame at log time (which is
#             # GADGET_ALLOWED_DURATION seconds later). Same re-seek approach
#             # as seat_absence below.
#             onset_frame = self._grab_frame_at(self.source, event_local)
#             if onset_frame is None:
#                 onset_frame = annotated

#             self.vstore.record_violation(
#                 annotated_frame=onset_frame, original_frame=frame,
#                 video_time=event_global, frame_index=global_frame,
#                 event_type="phone_use", severity="CRITICAL",
#                 confidence=conf, risk_score=80, risk_level="CRITICAL",
#                 factors=["phone_use", "distraction"], duration=dur,
#                 source_filename=self.source_filename, local_video_time=event_local,
#             )
#             log_distraction(self.logger, event_global, event="One of the pilots is using a mobile phone", severity="CRITICAL", frame=onset_frame)
...
#             # FIX — save the frame at the ACTUAL violation onset, not the
#             # frame at detection time (which is ABSENCE_ALLOWED_DURATION
#             # seconds late, since should_log() only fires after the pilot
#             # has been continuously absent for that long).
#             #
#             # `annotated`/`frame` here are the CURRENT live frame (T + 4s).
#             # The reader thread has already moved past T and discarded that
#             # frame, so the only way to recover it is to re-seek the source
#             # video file at event_local seconds. We do this immediately
#             # (rather than relying on extract_violation_frames()'s later
#             # pass) because passing annotated_frame would short-circuit that
#             # later seek-based recovery — Case 1 in analyzer.py uploads
#             # whatever annotated_frame it's given without re-seeking.
#             onset_frame = self._grab_frame_at(self.source, event_local)
#             if onset_frame is None:
#                 # Fallback: seek failed (e.g. very start of file) — use the
#                 # live frame rather than dropping the violation's evidence.
#                 onset_frame = annotated
...
#                 source_filename=self.source_filename, local_video_time=event_local,
#             )
#             log_distraction(self.logger, event_global, event="One of the pilots is away from the seat", severity="CRITICAL", frame=onset_frame)
...
#                 # FIX — save the frame at the moment the droop/sleep state
#                 # actually started, not the live frame at log time (which
#                 # is HEAD_DROP_DURATION seconds later). Same re-seek
#                 # approach as seat_absence/phone_use above.
#                 onset_frame = self._grab_frame_at(self.source, event_local)
#                 if onset_frame is None:
#                     onset_frame = annotated
...
#                     source_filename=self.source_filename, local_video_time=event_local,
#                 )
#                 log_distraction(self.logger, event_global, event=event_msg, severity="CRITICAL", frame=onset_frame)
...
class GadgetDetectionPipeline:
    ...
    def __init__(
        ...
        # ── Best-evidence frame cache for phone violations ────────────────────
        # key   = pilot_id (int)
        # value = (frame_copy: np.ndarray, confidence: float, video_time: float)
        #
        # During the GADGET_ALLOWED_DURATION (2 s) timer window we cache
        # whichever validated frame had the highest YOLO confidence for that
        # pilot's phone detection.  When the violation fires we use this frame
        # as the evidence image instead of a blind re-seek to onset time —
        # the re-seek was returning frames where no phone was visually present
        # because the timer started on a borderline/marginal detection.
        # The cache is cleared when the timer resets or the violation is logged.
        self._best_gadget_frame: dict = {}
    ...
    def _process_frame(self, frame, video_time, raw_frame_no, processed_frame_no):
        ...
        if run_gadget:
            ...
            # ── Update best-evidence cache ─────────────────────────────────────
            # For every pilot that has a validated phone hit this frame, record
            # the frame copy if it has the highest confidence seen so far in the
            # current timer window.  When the violation fires we use this frame
            # as the evidence image instead of a blind re-seek to onset time —
            # the re-seek was returning frames where no phone was visually present
            # because the timer started on a borderline/marginal detection.
            # The cache is cleared when the timer resets or the violation is logged.
            for r in results:
                if r.gadgets:
                    best_conf = max(g.confidence for g in r.gadgets)
                    cached    = self._best_gadget_frame.get(r.pilot_id)
                    if cached is None or best_conf > cached[1]:
                        self._best_gadget_frame[r.pilot_id] = (
                            frame.copy(), best_conf, video_time
                        )
                else:
                    # No validated phone this gadget frame — if the internal
                    # timer has also reset (miss tolerance exceeded), discard
                    # stale cached evidence from the previous episode.
                    timer = self.detector.timers.get(r.pilot_id)
                    if timer is None or timer.start_vtime is None:
                        self._best_gadget_frame.pop(r.pilot_id, None)
        ...
        if log_events:
            ...
            conf         = r_ref.gadgets[0].confidence if (r_ref and r_ref.gadgets) else 0.9
            dur          = r_ref.timer_value if r_ref else 0.0
            pid          = r_ref.pilot_id if r_ref else None
            event_global = max(0.0, global_time - GADGET_ALLOWED_DURATION)
            event_local  = max(0.0, video_time  - GADGET_ALLOWED_DURATION)

            # FIX — use the cached best-evidence frame (highest-confidence
            # validated phone detection during the 2 s timer window) instead
            # of a blind re-seek to onset time.
            #
            # The old re-seek approach grabbed whatever raw frame happened to
            # sit at (video_time - GADGET_ALLOWED_DURATION), which was often a
            # borderline/false YOLO hit that started the timer — resulting in
            # a saved image with no visible phone.
            #
            # The cache stores the frame where all filters (A, D, C1, C2, W)
            # passed AND YOLO confidence was highest, so the saved evidence
            # is guaranteed to show the phone near the pilot's ear.
            cached = self._best_gadget_frame.pop(pid, None) if pid is not None else None
            if cached is not None:
                onset_frame = cached[0]           # frame where phone was most clearly seen
                conf        = max(conf, cached[1]) # use the best confidence across the window
            else:
                # Fallback: cache missed (e.g. very start of file or pilot_id unknown)
                # — fall back to re-seek so we never silently drop evidence.
                onset_frame = self._grab_frame_at(self.source, event_local)
            if onset_frame is None:
                onset_frame = annotated
