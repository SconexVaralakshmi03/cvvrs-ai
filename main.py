...
class GadgetDetectionPipeline:
    ...
    def __init__(
        ...
        self.zone_manager     = DynamicZoneManager(stability_threshold_px=30.0)
    ...
    def run(self) -> str:
        ...
        # Explicit batch-run state resets
        self.zone_manager.reset()
        if hasattr(self.droop_detector, 'full_reset'):
            self.droop_detector.full_reset()
    ...
    def _process_frame(self, frame, video_time, raw_frame_no, processed_frame_no):
        ...
        if not self.zone_manager.is_calibrated and self._face_detector is not None:
            # Scale down for fast face detection
            h, w = frame.shape[:2]
            small_rgb = cv2.cvtColor(cv2.resize(frame, (w // 3, h // 3)), cv2.COLOR_BGR2RGB)
            res = self._face_detector.process(small_rgb)
            face_detected_this_frame = bool(res.detections)
            
        current_view = self.zone_manager.camera_view

        # ── Run gadget detector FIRST (synchronously) ──────────────────────────
        results:    List = []
        log_events: List = []
        if run_gadget:
            try:
                results, log_events = self.detector.process(
                    frame, round(global_time, 3), self.zone_manager, current_view, pose_landmarks_by_pilot
                )
            ...
            # Update zone manager with RAW person boxes
            if self.detector.last_frame_detections is not None:
                ...
                self.zone_manager.update(raw_person_boxes, face_detected_this_frame)
                current_view = self.zone_manager.camera_view
        ...
        future_droop   = self.executor.submit(self.droop_detector.process, frame, global_time, self._prev_frame_detections, self.zone_manager, current_view) if run_droop else None
