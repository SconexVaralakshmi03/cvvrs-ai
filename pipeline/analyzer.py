...
from schemas.models import VideoJob, VideoResult, ViolationResult
from services.s3_service import upload_frame, upload_frame_from_path, download_frame

# Import the existing pipeline and store — unchanged
from main import GadgetDetectionPipeline
from storage.violation_store import ViolationStore, _decode_frame, _encode_frame
...
import verification.llm_verifier as llm_verifier

# Fix 2/3 — durable per-video state/checkpoint store (see checkpoint_store.py)
import services.checkpoint_store as checkpoint_store
...
def _log_resource_snapshot(label: str, job_id: str = "", violations: int = -1) -> None:
    ...
    gpu_str = ""
    try:
        from services.resource_manager import get_gpu_memory_info
...
def analyze_journey(
    ...
    for idx, vj in enumerate(ordered):
        ...
        # environments.
        try:
            from services.resource_manager import is_gpu_over_threshold, gpu_cleanup, CUDA_ABORT_THRESHOLD
        ...
        try:
            from services.callback_client import is_resource_exhaustion_error, classify_video_error
