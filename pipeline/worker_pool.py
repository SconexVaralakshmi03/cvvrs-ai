...
from pipeline.journey_runner import run_worker_loop
...
class GPUWorkerPool:
    ...
    def submit(self, job_id: str, journey_id: int, folder_name: str,
               video_jobs: list, tmp_paths: Dict[int, str],
               initial_time_offset: float = 0.0,
               initial_frame_offset: int = 0) -> JourneyHandle:
        """
        ...
        Fix 7 — initial_time_offset/initial_frame_offset: non-zero when
        `video_jobs` is a RETRY submission covering only the videos that
        hadn't yet completed from a previous, watchdog-interrupted attempt
        at the same journey (see consumer.py's watchdog). Forwarded
        through to journey_runner.run_worker_loop() / analyze_journey() so
        journey-global violation timestamps stay continuous across the
        retry instead of restarting at zero.
        """
        worker = self.acquire()
        job_tuple = (job_id, journey_id, folder_name, video_jobs, tmp_paths,
                     initial_time_offset, initial_frame_offset)
        try:
            worker.job_queue.put(job_tuple)
        except Exception:
        ...
        log.info(
            "[WorkerPool] Journey job=%s dispatched to worker %d (pid=%s)%s.",
