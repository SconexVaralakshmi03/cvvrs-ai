...
def compute_journey_status(
    ...
    """
    Computes the journey-level terminal status. Only two terminal outcomes
    exist now — "Completed with Errors" has been removed:
        COMPLETED — every video succeeded
        FAILED    — ANY video failed, OR none succeeded, OR there were
                    zero videos to begin with

    A journey that finishes with even one failed video is now reported as
    FAILED (not a partial-success state), so the frontend and backend only
    ever see COMPLETED or FAILED.
