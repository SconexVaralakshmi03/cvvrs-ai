...
# ─────────────────────────────────────────────────────────────────────────────
#
# Resource-lifecycle notes (Phase 3 / Phase 4)
# ─────────────────────────────────────────────
# This model is a per-PROCESS singleton: loaded once, reused across every
# video and journey handled by this process. That's correct for speed —
# reloading YOLO weights per video would be wasteful — but it means this
# module is also the only place that knows how to release the model's GPU
# memory. release_model() below is that release hook, called by
# resource_manager.py at the end of every journey (success or failure) and
# during emergency/shutdown cleanup.
#
# _get_model() additionally:
#   - pins the model to an explicit, env-overridable device instead of
#     letting ultralytics pick a default
#   - clears any stale CUDA cache before allocating, in case a previous
#     load in this same process (or a just-finished release_model() call)
#     left anything behind
#
# NOTE on half precision: an earlier version of this fix cast the model to
# fp16 via model.model.half() to reduce VRAM footprint. That broke real
# inference with "expected mat1 and mat2 to have the same dtype, but got:
# struct c10::Half != float" — ultralytics' AutoBackend fuses conv+bn
# layers lazily on the FIRST predict() call (engine/predictor.py
# setup_model() → model.fuse()), and that fuse step's internal arithmetic
# (torch_utils.py fuse_conv_and_bn) is not dtype-matched for fp16 weights.
# Casting before that first call corrupts the model. Rather than chase the
# exact warm-up-inference ordering needed to fuse-then-cast safely, we
# simply don't cast to half — fp32 is the safe, correct default, and the
# VRAM savings were a secondary nice-to-have on top of the real OOM fixes
# (empty_cache() before load, explicit device placement, release_model()
# at journey end), not load-bearing for them.

import os as _os

_model = None
_MODEL_DEVICE = _os.environ.get("GADGET_YOLO_DEVICE", "cuda:0")
    ...
    if _model is None:
        ...
        if _MODEL_DEVICE.startswith("cuda") and _has_cuda:
            try:
                ...
                logger.warning(
                    "YOLO device placement failed — continuing with "
                    "default device", exc_info=True,
                )
        # If no CUDA, leave on default (CPU) device.
...
def release_model() -> None:
    """
    Explicitly drop the YOLO singleton and free its CUDA memory.

    Called by resource_manager.py's cleanup_after_journey() /
    cleanup_after_failure() / emergency_cleanup() / cleanup_on_shutdown().
    Must be called from the SAME process that called _get_model() — in the
    subprocess architecture that means once, at the end of each journey's
    child process, right before it returns/exits. Safe to call multiple
    times and safe to call even if the model was never loaded.
    """
    global _model
    if _model is not None:
        ...
        try:
            ...
            logger.warning("CUDA cache release failed in release_model()", exc_info=True)
...
class _PilotTimer:
    ...
    # any existing threshold/relog/miss-tolerance behaviour above). Set
    # True the moment this episode is first confirmed/logged.
    ...
    # NEW — call instead of reset() at the point the episode genuinely
    # ends (gadget no longer detected, miss-tolerance exceeded). If this
    # episode was confirmed at least once, returns
    # (start_vtime, end_vtime, true_duration); otherwise None. Always
    # resets afterward — same effect as the old bare `timer.reset()`
    # plus this extra return value.
    def close_if_confirmed(self, end_video_time: float) -> "Optional[Tuple[float, float, float]]":
        ...
        if self.confirmed and self.start_vtime is not None:
            ...
            print(f"[TIMER-CLOSE][gadget] pilot={self.pilot_id} start={self.start_vtime:.2f} "
                  f"end={end_video_time:.2f} true_dur={result[2]:.2f}")
        else:
            print(f"[TIMER-CLOSE][gadget] pilot={self.pilot_id} skipped — "
...
class GadgetDetector:
    ...
    def process(
    ...
    ) -> Tuple[List[PilotResult], List[Tuple[int, str]], List[Tuple[int, float, float, float]]]:
        """
        ...
        completed_events : List[(pilot_id, start_vtime, end_vtime, true_duration)]
                            — NEW, additive: emitted once per confirmed episode,
                            the frame it actually ends (phone no longer seen
                            past miss tolerance).
        """
        ...
        # for every phone-usage episode that gets confirmed (logged) and
        # then actually ends (phone no longer detected past miss
        # tolerance) within this video. Doesn't change existing behaviour.
