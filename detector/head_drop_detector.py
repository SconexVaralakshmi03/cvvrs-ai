...
# # #     # Alert timer (video_time, not wall clock — FIX)
...
# # #     # FIX (Video time): mirrors _AbsenceTimer / _PilotTimer — timer must run
# # #     # on video_time, not wall clock. Otherwise a video processed faster or
# # #     # slower than real-time (batch mode, time_offset/frame_offset) drifts
# # #     # away from the intended HEAD_DROP_DURATION in *video* seconds.
...
# # #         if self.alert_start is None:
# # #             return 0.0
# # #         return max(0.0, video_time - self.alert_start)
...
# # #     def close(self) -> None:
# # #         """
# # #         Release any resources owned by this HeadDroopDetector instance.

# # #         Unlike GadgetDetector/SeatAbsenceDetector (pure-Python state),
# # #         HeadDroopDetector owns two real MediaPipe graphs — self._pose and
# # #         self._face — created in __init__. Each holds native
# # #         (non-garbage-collected) resources that must be explicitly closed,
# # #         or repeated per-video instantiation across a journey leaks native
# # #         memory/handles (this is what the resource_manager's
# # #         RSS/threads/handles snapshot logging is watching for).

# # #         This method exists so main.py's finally block can call
# # #         self.droop_detector.close() symmetrically alongside
# # #         self.detector.close() and self.absence_detector.close() without
# # #         raising AttributeError. Safe to call multiple times.
# # #         """
# # #         for graph in (getattr(self, "_pose", None), getattr(self, "_face", None)):
# # #             if graph is not None:
# # #                 try:
# # #                     graph.close()
# # #                 except Exception as exc:
# # #                     print(f"[HeadDroopDetector] close() failed for {graph!r}: {exc}")

# # #         self._pose = None
# # #         self._face = None

# # #         # Reset per-pilot state so no references to old frames/scores linger.
# # #         self._states = {1: _PilotState(1), 2: _PilotState(2)}
...
#     def close(self) -> None:
#         """
#         Release the two native MediaPipe graphs (_pose, _face) created in
#         __init__. A fresh HeadDroopDetector is instantiated for every
#         single video processed (see analyzer.py / main.py's per-video
#         GadgetDetectionPipeline construction), so without this, every
#         video leaks two MediaPipe graph handles -- harmless for a
#         short-lived one-journey-per-process worker, but a real, unbounded
#         leak for a persistent GPU worker process that lives for the
#         lifetime of the service and processes thousands of videos.

#         Safe to call multiple times. Never raises -- main.py's
#         _release_pipeline_resources() calls this from a finally block and
#         a failure here must never prevent the rest of that cleanup from
#         running.
#         """
#         for graph in (getattr(self, "_pose", None), getattr(self, "_face", None)):
#             if graph is not None:
...
class _PilotState:
    ...
    # any existing wall-clock alert-timer/relog behaviour above).
    # alert_start_vtime mirrors alert_start but on the VIDEO timeline
    # (alert_start itself is wall-clock, which is what the rest of this
    # class already uses for its own timer — kept as-is). confirmed is
    # set True the moment this episode is first logged.
    alert_start_vtime: Optional[float] = None
    last_droop_type:    Optional[str]  = None
    ...
    def activate_alert(self, video_time: Optional[float] = None):
        self.miss_frames = 0
        if self.alert_start is None:
            self.alert_start = time.monotonic()
        # NEW — additive: mirror the same "first activation only" rule on
        # the video timeline so we have a start point comparable to the
        # timestamps already reported elsewhere in the pipeline.
        if video_time is not None and self.alert_start_vtime is None:
            self.alert_start_vtime = video_time

    def reset_alert(self):
        self.alert_start = None
        self.last_logged = None
        self.alert_start_vtime = None
    ...
    # NEW — call instead of reset_alert() at the point the episode
    # genuinely ends (drowsy_signal goes False, pilot stands up, or the
    # pilot disappears). If this episode was confirmed (logged) at least
    # once, returns (start_vtime, end_vtime, true_duration, droop_type);
    # otherwise None. Always resets afterward — same effect as the old
    # bare `reset_alert()` plus this extra return value.
    def close_alert_if_confirmed(self, end_video_time: float) -> "Optional[Tuple[float, float, float, str]]":
        result = None
        if self.confirmed and self.alert_start_vtime is not None:
            result = (
                self.alert_start_vtime,
                end_video_time,
                max(0.0, end_video_time - self.alert_start_vtime),
                self.last_droop_type or "UNKNOWN",
            )
            print(f"[TIMER-CLOSE][droop] pilot={self.pilot_id} start={self.alert_start_vtime:.2f} "
                  f"end={end_video_time:.2f} true_dur={result[2]:.2f} type={result[3]}")
        else:
            print(f"[TIMER-CLOSE][droop] pilot={self.pilot_id} skipped — "
                  f"confirmed={self.confirmed} alert_start_vtime={self.alert_start_vtime}")
        self.reset_alert()
        return result
...
class HeadDroopDetector:
    ...
    def process(
        ...
        video_time:       float,
        frame_detections: Optional[FrameDetections],
    ) -> Tuple[List[DroopResult], List[Tuple[int, str]], List[Tuple[int, float, float, float, str]]]:

        results:    List[DroopResult]     = []
        log_events: List[Tuple[int, str]] = []
        # NEW — additive: (pilot_id, start_vtime, end_vtime, true_duration,
        # droop_type) for every drowsy/droop episode that gets confirmed
        # (logged) and then actually ends within this video. Doesn't
        # change any existing return value or behaviour.
        ...
        for pid in (1, 2):
            ...
            if crop_data is None:
                ...
                if (state.alert_start is not None
                    ...
                    closed = state.close_alert_if_confirmed(video_time)
                    if closed is not None:
                        start_v, end_v, true_dur, dtype = closed
                        completed_events.append((pid, start_v, end_v, true_dur, dtype))
                    state.full_reset()
                    results.append(DroopResult(pid))
                    continue
            else:
...
# ─────────────────────────────────────────
    ...
    # ✅ append once (VERY IMPORTANT)
            ...
            if drooping:
                # NEW — additive: remember the posture type so that if/when
                # this episode later closes (close_alert_if_confirmed), the
                # completed event can report which kind of droop it was.
                state.last_droop_type = droop_type
                if (state.last_logged is None or (current_time - state.last_logged) >= RELOG_INTERVAL):
                    if state.last_logged_second != current_second:
                        log_events.append((pid, f"DROWSINESS:{droop_type}"))
                        state.last_logged = current_time
